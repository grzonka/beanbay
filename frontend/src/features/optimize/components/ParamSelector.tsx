...
interface Props { params: string[]; xValue: string; yValue: string; onXChange: (v: string) => void; onYChange: (v: string) => void; }
    ...
    <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
      <FormControl size="small" sx={{ minWidth: 160 }}>
        ...
        <Select value={xValue} label="X Axis" onChange={(e) => onXChange(e.target.value)}>
          {params.map((p) => <MenuItem key={p} value={p}>{p.replace(/_/g, ' ')}</MenuItem>)}
        </Select>
      </FormControl>
      <FormControl size="small" sx={{ minWidth: 160 }}>
